...
/**
 * 订单在 Redis 中的缓存副本投影（读模型）：与 {@link OrderEsProjection} 平行、独立，
 * 字段为其全量信息剔除 ES 检索派生字段 {@code itemProductNames} / {@code itemProductNamesText}。
 * 仅作为读模型数据容器，不含任何存储与计算逻辑。
 *
 * @author wizard-lee
 */
@Data
public class OrderCacheProjection implements IOrderProjection {
