...
Pragmatic DDD 是一个基于 Java 17 的**实用主义**领域驱动设计（DDD）框架，把 DDD 战术模式（实体、值对象、聚合根、领域事件、仓储、应用服务）做成了开箱即用的 Java 基类与接口。

它解决的核心问题是：**在 Java 项目中标准化地表达 DDD 战术模式，不绑死任何基础设施框架。**
