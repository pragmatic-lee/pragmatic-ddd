...
传统 DDD 落地时，开发者容易在领域层里写满"驱动代码"：在哪里触发规则校验、如何收集并发布事件、事务边界怎么圈定、跨聚合怎么协调。这些代码与业务无关，却散布在每个聚合、每个应用服务里，重复且易错，最终稀释了领域层的纯度。

pragmatic-ddd 的立场是：**领域层不应当是"执行者"，而应当是"指挥中心"**。聚合根负责描述"发生了什么、要做什么、有哪些约束"，而"何时执行、按什么顺序、失败如何兜底"这类执行职责，交给框架托管。
