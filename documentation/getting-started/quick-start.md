...
::: warning 前置：当前版本尚未发布到 Maven 中央仓库
`pragmatic-ddd` 目前**未发布到中央仓库**，直接引入坐标会拉取失败。请先完成下面的 **§0 本地安装**，将框架安装到你本地的 `~/.m2` 仓库后，再按 §1 引入依赖即可正常使用。
:::
...
> 该脚本还支持选择性安装与连带示例构建，详见仓库根目录 `install-local.sh` 头部注释，或查看 [仓库安装到本地仓库说明](../../README.md#安装到本地仓库)。
