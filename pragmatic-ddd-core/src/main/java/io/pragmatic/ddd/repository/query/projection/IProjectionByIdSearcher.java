...
/**
 * 按主键 / 批量主键直取投影的检索器，与按条件检索的 {@link IOneQuerySearcher} 等平级。
 * 不依赖任何条件族（无 QueryCriteria），由各集成模块（ES / Redis / 读表连接器）实现，core 只定义中立接口。
 *
 * <p>其服务的投影类型是<b>索引级全量投影类型</b>——对齐某物理存储索引文档形状的
 * 具体投影类，而非投影体系接口；子投影由 {@link IReducer} 在 Java 内存中二次裁剪。</p>
 *
 * @param <P> 投影类型
 * @author wizard-lee
 */
